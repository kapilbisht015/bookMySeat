...
<div class="filters-container">
  ...
  <form method="GET" action="events.php">
    ...
    <div class="filter-group">
      <h4>Category</h4>
      <button type="submit" name="shows" value="comedy shows">Comedy Shows</button>
      <button type="submit" name="shows" value="music shows">Music Shows</button>
      <button type="submit" name="shows" value="kids">kids</button>
      <button type="submit" name="shows" value="performances">Performances</button>
      <button type="submit" name="date" value="">All</button>
    </div>
  </form>
</div>

     <h1>Events in Delhi</h1>
<section class="card-container">
<?php
    // Base query
  $sql = "SELECT * FROM events WHERE city = 'Delhi'";

  // filter
  if (!empty($_GET['shows'])) {
      $shows = $conn->real_escape_string($_GET['shows']);
      $sql .= " AND category = '$shows'";
  }
  ...
  if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
          echo "<a href='event_booking.php?id=" . $row['id'] . "' class='card'>";
